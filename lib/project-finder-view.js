    ...
    return $$(function () {
      this.li({class: showPath ? 'two-lines' : 'one-line'}, () => {
        ...
        this.div({class: `primary-line ${primaryIconClass}`}, () => {
          ...
          if (!showPath && item.paths.length > 1) {
            this.span({class: 'more-paths'}, `(${item.paths.length} paths)`)
          }
        })
        ...
        if (showPath) {
          ...
          this.div({class: `secondary-line ${secondaryIconClass}`}, () => {
            this.text(tildify(item.paths[0]))

            if (item.paths.length > 1) {
              this.span({class: 'more-paths'}, `+${item.paths.length - 1} more`)
            }
          })
        }
      })
